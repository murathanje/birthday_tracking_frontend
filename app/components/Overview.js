              ...
              <div className="flex gap-2 sm:gap-3 px-2 sm:px-4">
                {categories.map(category => (
                  <button
                    ...
                    className={`
                      group relative shrink-0 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-medium transition-all duration-300 ease-in-out
                    ...
                    `}
                  >
                    <div className="flex items-center gap-1 sm:gap-2.5 whitespace-nowrap">
                      <span className="text-sm sm:text-lg">{categoryIcons[category]}</span>
          ...
          {sortedAndFilteredFriends.map((friend) => {
            ...
            return (
              <div
                ...
                className={`group relative bg-white/80 backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-soft hover:shadow-soft-lg transition-all duration-300 cursor-pointer ${
                  isUpcoming ? 'ring-2 ring-black/20' : 'border border-slate-200/50'
                } ${isPast ? 'opacity-60' : ''}`}
              >
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    <h3 className="text-base sm:text-lg font-semibold text-black group-hover:text-black/70">
                      {friend.name}
                    </h3>
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <span className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full bg-slate-100 text-black/70 capitalize flex items-center gap-1.5">
                        <span className="text-xs sm:text-sm">{categoryIcons[friend.category]}</span>
                        {friend.category}
                      </span>
                    </div>
                  </div>
            ...
            <div className="text-center py-12 bg-white/80 backdrop-blur-sm rounded-2xl shadow-soft border border-slate-200/50">
              ...
              <h3 className="text-lg font-semibold text-black mb-2">
                {searchQuery || selectedCategory !== 'all' 
                  ? "No birthdays found"
                  : "Welcome! Let's add your first birthday"}
              </h3>
              <p className="text-black/60">
                {searchQuery || selectedCategory !== 'all'
                  ? "Try adjusting your search or filters"
                  : "Click the + button above to start tracking birthdays"}
              </p>
            </div>
          )}
        </div>
